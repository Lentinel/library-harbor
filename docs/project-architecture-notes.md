...
This repository is a modular Spring application for a public library. It contains
two bounded contexts: catalogue management and lending. The lending context models
patrons, books, holds, checkouts, overdue work, daily sheets, and a patron profile
read model. Domain events connect aggregate changes with projections and application
services.

The project is intentionally organized as a small modular monolith. Domain logic is
kept separate from persistence and web adapters, while the existing tests exercise
both isolated business rules and database-backed event propagation.

The main production code is under `src/main/java`; unit and integration tests are
under `src/test/groovy` and `src/integration-test/groovy`. The project uses Maven
and Java 11.

# Project notes

The original design notes and examples remain below as historical project material.
They describe the domain vocabulary, but are not a substitute for reading the
current implementation and its tests.
...
![Event Storming Domain description](images/eventstorming-domain-desc.png)   
The EventStorming session led us to numerous discoveries, modeled with the sticky notes:  
![Event Storming Big Picture](images/eventstorming-big-picture.jpg)   
During the session we discovered following definitions:  
![Event Storming Definitions](images/eventstorming-definitions.png)    
...
![Example mapping](images/eventstorming-design-level.jpg)  

Please follow the links below to get more details on each of the mentioned steps:
- [Big Picture EventStorming](./big-picture.md)
...
![Architecture](images/architecture-big-picture.png) 
...
![Component diagram](c4/component-diagram.png)
...
#### Placing on hold
![Placing on hold](images/placing_on_hold.jpg)
...
![Restricted book policy](images/placing-on-hold-policy-restricted.png)
```java
...
![Overdue checkouts policy](images/placing-on-hold-policy-overdue.png)
...
![Max number of holds policy](images/placing-on-hold-policy-max.png)
...
![Open ended hold policy](images/placing-on-hold-policy-open-ended.png)
